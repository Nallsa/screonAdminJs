...
export const useScheduleStore = create<ScheduleState, [["zustand/immer", never]]>(
    immer((set, get) => {
        ...
        return {
            ...
            setPriority: (p) => set(s => { s.priority = p }),

            hoveredBlock: null,
            setHoveredBlock: (b) => set(s => { s.hoveredBlock = b }),
            ...
            selectedPlaylist: '',
            setSelectedPlaylist: plId => set(s => { s.selectedPlaylist = plId }),
            ...
            addBlock: () => {
                const { selectedScreens, currentWeek, startTime, endTime, selectedPlaylist, isFixedSchedule, selectedDays } = get()
                ...
                selectedScreens.forEach(screenId => {
                    ...
                    selectedDays.forEach(dayShort => {
                        ...
                        set(s => { (s as any)[mapKey][screenId].push(b) })
                    })
                })
            },
            ...
            sendSchedule: async () => {
                const { selectedScreens, isFixedSchedule, isRecurring, priority, scheduledFixedMap, scheduledCalendarMap, scheduleId } = get()
                ...
                try {
                    if (scheduleId) {
                        ...
                        set(s => { s.scheduleId = res.data.id })
                    }
                } catch (e: any) {
            ...
            getSchedule: async () => {
                ...
                set(s => { s.selectedScreens = Array.from(screens); s.isRecurring = data.recurring; s.priority = data.priority })
            },

            onDateSelected: d => set(s => { s.selectedDate = d; s.currentWeek = getCurrentWeekByDate(d) }),
            toggleFixedSchedule: () => set(s => { s.isFixedSchedule = !s.isFixedSchedule }),
            togglePlayRecurring: () => set(s => { s.isRecurring = !s.isRecurring }),
            toggleShowBackground: () => set(s => { s.isShowBackground = !s.isShowBackground }),
            setStartTime: t => set(s => { s.startTime = normalizeTime(t) }),
            setEndTime: t => set(s => { s.endTime = normalizeTime(t) }),

            showMode: 'cycle',
            cycleMinutes: 10,
            pauseMinutes: 50,
            intervalMinutes: 60,
            setShowMode: m => set(s => { s.showMode = m }),
            setCycleMinutes: m => set(s => { s.cycleMinutes = m }),
            setPauseMinutes: m => set(s => { s.pauseMinutes = m }),
            setIntervalMinutes: m => set(s => { s.intervalMinutes = m }),

            maxPerDay: 0,
            maxPerHour: 0,
            maxTotalDuration: 0,
            setMaxPerDay: n => set(s => { s.maxPerDay = n }),
            setMaxPerHour: n => set(s => { s.maxPerHour = n }),
            setMaxTotalDuration: n => set(s => { s.maxTotalDuration = n }),
        }
    })
)
